...
const GestionoContext = createContext<GestionoContextProps | undefined>(undefined);

export const GestionoProvider = ({ children }: { children: ReactNode }) => {
    const [divisions, setDivisions] = useState<GestionoDivision[]>([]);
    const [pendingRecords, setPendingRecords] = useState<V2GetPendingRecordsResponse | null>(null);
    ...
    const fetchPendingRecords = async () => {
        ...
        try {
            ...
            setError(err.message || 'Error desconocido al cargar registros pendientes');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <GestionoContext.Provider value={{ divisions, pendingRecords, isLoading, error, refreshDivisions, refreshPendingRecords }}>
            {children}
