...
export type InvoiceState = 'PENDING' | 'PAID' | 'PARTIALLY_PAID' | 'OVERDUE' | 'DRAFT' | 'CANCELED'; // Inferred common states
...
export interface GestionoBeneficiary {
    ...
    name: string;
    organizationId: number;
    type: 'CLIENT' | 'PROVIDER' | 'SELLER' | 'ORGANIZATION' | 'BOTH' | 'EMPLOYEE' | 'OTHER';
...
export interface GestionoDivision {
    ...
    metadata?: {
        ...
        budgetPeriod?: 'MONTHLY' | 'YEARLY' | 'QUARTERLY' | 'WEEKLY' | 'DAILY' | 'LIFETIME';
        sellTarget?: number;
        sellTargetCurrency?: 'DOP' | 'USD' | 'EUR';
        sellTargetPeriod?: 'MONTHLY' | 'YEARLY' | 'QUARTERLY' | 'WEEKLY' | 'DAILY' | 'LIFETIME';
        [key: string]: any;
    };
}
...
export interface CreateBeneficiaryBody {
    name: string;
    type: 'CLIENT' | 'PROVIDER' | 'ORGANIZATION' | 'EMPLOYEE' | 'SELLER' | 'GOVERNMENT';
    taxId?: string;
    reference?: string;
    labels?: string[];
    creditLimit?: number;
...
export type AdvancedSearchFilter = {
    ...
    method: '=' | '>' | '<' | 'in' | 'not in' | '!=' | 'like' | 'not like' | 'is null' | 'is not null';
}
