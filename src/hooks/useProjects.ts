    ...
    const fetchProjects = async () => {
        ...
        try {
            ...
            const transformed: Project[] = (data || []).map((item: any) => {
                ...
                const completionPercentage = totalValue > 0
                ...
                const endDate = new Date(new Date(startDate).setFullYear(new Date(startDate).getFullYear() + 1)).toISOString(); // Mock 1 year duration
                ...
                return {
                    ...
                    executedBudget,
                    status,
                    location: item.locales ? `Nivel ${item.locales.level}` : "Sin ubicación",
