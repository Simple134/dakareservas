...
const GESTIONO_ORGANIZATION_ID = process.env.NEXT_PUBLIC_GESTIONO_ORGANIZATION_ID || '';
...
export function validateGestionoConfig(): { valid: boolean; errors: string[] } {
    ...
    if (!GESTIONO_API_PUBLIC_KEY) errors.push('GESTIONO_API_PUBLIC_KEY no está configurado');
    if (!GESTIONO_API_PRIVATE_KEY) errors.push('GESTIONO_API_PRIVATE_KEY no está configurado');
    if (!GESTIONO_ORGANIZATION_ID) errors.push('GESTIONO_ORGANIZATION_ID no está configurado');
