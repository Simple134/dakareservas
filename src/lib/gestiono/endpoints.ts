...
export async function getInvoice(invoiceId: string): Promise<GestionoInvoiceResponse> {
...
export async function getPendingRecords(params: Record<string, any> = {}): Promise<any> {
    ...
    const path = queryString ? `/v2/record/pending?${queryString}` : '/v2/record/pending';
...
export async function getInvoiceEvents(invoiceId: string): Promise<GestionoEvent[]> {
...
export async function getBeneficiaries(params: BeneficiaryQueryParams = {}): Promise<any> {
    const queryParams = new URLSearchParams();
    Object.entries(params).forEach(([key, value]) => {
...
export async function addBeneficiary(params: CreateBeneficiaryBody): Promise<any> {
...
export async function updateBeneficiary(params: BeneficiaryContactResponse): Promise<any> {
    return gestionoRequest<any>(`/v1/beneficiary/`, {
        method: 'PATCH',
...
export async function getBeneficiaryById(beneficiaryId: string | number): Promise<any> {
    return gestionoRequest<any>(`/v1/beneficiary/${beneficiaryId}`, {
        method: 'GET',
    });
}

export async function createBeneficiary(data: GestionoBeneficiaryPayload): Promise<any> {
    return gestionoRequest<any>('/v1/beneficiary', {
        method: 'POST',
        body: JSON.stringify(data),
    });
...
export function transformToGestionoFormat(invoice: any, divisionId: number = 1): GestionoRecordPayload {
...
export async function getDivisionById(id: number): Promise<GestionoDivisionWithBalance[]> {
    return gestionoRequest<GestionoDivisionWithBalance[]>(`/v1/division/${id}`, {
        method: 'GET',
    });
}

export async function postDivision(data: GestionoDivisionPayload): Promise<GestionoDivision> {
