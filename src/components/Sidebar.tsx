      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div>
          ...
          <div className="space-y-2">
            <DetailRow
              ...
              value={selectedReservation.client_name}
            />
            <DetailRow
              label={selectedReservation.identification_label}
              value={selectedReservation.identification_value}
            />
            ...
            {selectedReservation.raw_fisica && (
              <>
                <DetailRow
                  label="Género"
                  value={selectedReservation.raw_fisica.gender || "-"}
                />
                <DetailRow
                  label="Nacionalidad"
                  value={selectedReservation.raw_fisica.nationality || "-"}
                />
                <DetailRow
                  label="Estado Civil"
                  value={selectedReservation.raw_fisica.marital_status || "-"}
                />
                <DetailRow
                  label="Ocupación"
                  value={selectedReservation.raw_fisica.occupation || "-"}
                />
              </>
            )}
            ...
            {selectedReservation.raw_juridica && (
              <>
                <DetailRow
                  label="Tipo Empresa"
                  value={selectedReservation.raw_juridica.company_type || "-"}
                />
                <DetailRow
                  label="Rep. Legal"
                  value={selectedReservation.raw_juridica.rep_name || "-"}
                />
              </>
            )}
          </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            <p className="text-sm text-gray-900">
              {selectedReservation.address_display}
            </p>
          </div>
        </div>
            ...
            <div className="space-y-2">
              ...
              {selectedReservation.locale_details ? (
                <>
                  <DetailRow
                    label="Nivel"
                    value={selectedReservation.locale_details.level.toString()}
                  />
                  <DetailRow
                    label="Área"
                    value={`${selectedReservation.locale_details.area_mt2} mt²`}
                  />
                  <DetailRow
                    label="Precio / mt²"
                    value={new Intl.NumberFormat("en-US", {
                      style: "currency",
                      currency: "USD",
                    }).format(selectedReservation.locale_details.price_per_mt2)}
                  />
                  <DetailRow
                    label="Valor Total"
                    value={new Intl.NumberFormat("en-US", {
                      style: "currency",
                      currency: "USD",
                    }).format(selectedReservation.locale_details.total_value)}
                    highlight
                  />
                  <DetailRow
                    label="Estado Actual"
                    value={selectedReservation.locale_details.status}
                  />
                </>
              ) : (
                <p className="text-sm text-yellow-600 italic">
                  Detalles adicionales del local no disponibles en la base de
                  datos.
                </p>
              )}
            </div>
        ...
        <div>
          ...
          <div className="space-y-2">
                    ...
                    <>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xs font-bold text-gray-900">
                          1 USD = {exchangeRate.toFixed(4)} DOP
                        </span>
                      </div>
            ...
            {/* Payments List */}
            <div className="mt-4">
              <h4 className="text-sm font-bold text-black mb-2 uppercase border-b border-gray-200 pb-1">
                Historial de Pagos
              </h4>
              {payments.length === 0 ? (
                ...
                <div className="space-y-2">
                  {payments
                    ...
                    .map((p) => (
                      <div
                        key={p.id}
                        className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm flex flex-col gap-2"
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                handleUpdatePaymentMethod(p.id, e.target.value)
                              }
                        ...
                        <div className="flex flex-col items-end gap-1">
                          <div className="flex items-center gap-1">
                            <select
                              ...
                              onChange={(e) =>
                                handleUpdatePaymentStatus(p.id, e.target.value)
                              }
                              className={`text-[10px] font-bold px-2 py-1 rounded border-0 cursor-pointer focus:ring-1 focus:ring-offset-1 ${
                                p.status === "approved"
                      ...
                      {payment.receipt_url!.toLowerCase().endsWith(".pdf") ? (
                        ...
                        >
                          ...
                          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center">
                            <span className="text-white opacity-0 group-hover:opacity-100 text-xs font-bold bg-black/50 px-2 py-1 rounded">
                              Ver
                            </span>
                          </div>
                        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          <div>
            ...
            <p className="font-bold text-gray-900">
              ${selectedLocale.price_per_mt2?.toLocaleString()}
            </p>
          </div>
          ...
          <div className="col-span-2">
            <span
              className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${
                selectedLocale.status?.toLowerCase().includes("disponible")
                  ? "bg-green-100 text-green-800"
                  : selectedLocale.status?.toLowerCase().includes("vendido")
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {isEditing ? (
          ...
          <div className="space-y-4">
            ...
            <DetailRow
              label={selectedUser.identification_label}
...
export const SidebarPayment = ({
  ...
  const [selectedCurrency, setSelectedCurrency] = useState<"USD" | "DOP">(
    "USD",
  );
  const [exchangeRate, setExchangeRate] = useState<number | null>(null);
  const [loadingRate, setLoadingRate] = useState(false);
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 space-y-4">
          <p className="text-sm text-gray-600 mb-4">
            Ingrese el monto que desea abonar y suba el comprobante de la
            transacción.
          </p>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
              {loadingRate ? (
                  ...
                  {amount && parseFloat(amount) > 0 && (
                    <div className="pt-2 border-t border-blue-300">
                      ...
                      <p className="text-xs text-gray-600 mt-1">
                        {parseFloat(amount).toLocaleString("es-DO")} DOP = $
                        {usdAmount.toFixed(2)} USD
                      </p>
                    </div>
                  )}
                ...
                <p className="text-sm text-red-600">
                  Error al obtener la tasa de cambio. Intente nuevamente.
                </p>
              )}
            </div>
          )}
