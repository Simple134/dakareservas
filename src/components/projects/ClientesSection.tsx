...
export function ClientesSection() {
    ...
    const getFilteredClients = () => {
        ...
        return clientsData.filter((client) => {
            ...
            const identification = client.data?.identification?.toString().toLowerCase() || "";
        ...
        <CustomCard className="p-6">
            ...
            {searchTerm && (
                <div className="mb-4 text-sm text-gray-600">
                    Mostrando {getFilteredClients().length} de {clientsData.length} clientes
                </div>
            )}
                                ...
                                <tr key={client.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <div className={`shrink-0 h-10 w-10 rounded-full flex items-center justify-center ${isCompany(client) ? "bg-purple-100" : "bg-blue-100"
                                                }`}>
                                                {isCompany(client) ? (
                                    ...
                                    <td className="px-4 py-4 whitespace-nowrap">
                                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${client.data?.status === "active"
                                            ? "bg-green-100 text-green-800"
                                            : "bg-yellow-100 text-yellow-800"
                                            }`}>
                                            {client.data?.status === "active" ? "Activo" : "Pendiente"}
                                        </span>
                                    </td>
                                </tr>
                            ))}
            ...
            {/* No Results State */}
            {!isLoading && clientsData.length > 0 && getFilteredClients().length === 0 && (
                <div className="text-center py-12 text-gray-500">
                    <Users className="w-12 h-12 mx-auto mb-3 text-gray-400" />
