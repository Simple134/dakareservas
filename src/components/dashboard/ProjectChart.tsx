...
export function ProjectChart() {
    ...
    const statusData = [
        ...
        { name: 'Completado', value: projects.filter(p => p.status === 'completed').length, color: '#3b82f6' }
                    ...
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={budgetData}>
                                ...
                                <Tooltip
                                    formatter={(value: number | undefined) => [`$${((value || 0) * 1000).toLocaleString()}`, '']}
                                    labelFormatter={(label) => `Proyecto: ${label}`}
                                />
                                <Bar dataKey="presupuesto" fill="#3b82f6" name="Presupuesto" radius={[4, 4, 0, 0]} />
                                <Bar dataKey="ejecutado" fill="#10b981" name="Ejecutado" radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </CardContent>
