  ...
  const onSubmit = async (data: PersonnelFormData) => {
    ...
    try {
      ...
      if (searchResponse.ok) {
        ...
        if (existingBeneficiary) {
          ...
          if (!createBeneficiaryResponse.ok) {
            const errorData = await createBeneficiaryResponse
              .json()
              .catch(() => ({}));
            console.error("❌ Beneficiary creation error:", errorData);
            throw new Error(
              errorData.details ||
                errorData.error ||
      ...
      const payrollResponse = await fetch(
        "/api/gestiono/pendingRecord/create",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!payrollResponse.ok) {
        ...
        throw new Error(
          errorData.details ||
            errorData.error ||
            "Error al crear el registro de nómina",
        );
      }
