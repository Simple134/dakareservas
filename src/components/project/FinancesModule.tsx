...
export function FinancesModule({
  ...
  const [isLoading, setIsLoading] = useState(!providedRecords);
  const [error, setError] = useState<string | null>(null);

  // Fetch pending records for this division
  useEffect(() => {
    if (providedRecords) {
    ...
    fetchRecords();
  }, [projectId, providedRecords]);

  const handleInvoiceCreated = () => {
    // Refresh records after creating a new invoice
                      ...
                      {filteredRecords.length === 0 ? (
                        ...
                        filteredRecords.map((r) => (
                          <TableRow key={r.id} className="hover:bg-gray-50/50">
                            ...
                            <TableCell>
                              <Badge
                                variant={r.isSell ? "default" : "secondary"}
                              >
