...
import type { GestionoInvoiceItem, PendingRecordElement, GestionoBeneficiary } from '@/src/types/gestiono';
...
export async function generateQuotePDF(data: QuotePDFData) {
    ...
    const clientPhone = beneficiary?.contacts?.find(c => c.type === 'phone')?.data || '';
    const clientAddress = beneficiary?.contacts?.find(c => c.type === 'address')?.data || '';
    ...
    if (clientAddress) {
        yPosition -= 15;
        const addressText = clientAddress.length > 60 ? clientAddress.substring(0, 60) + '...' : clientAddress;
    ...
    // Table Headers
    const tableHeaders = ['Ref.', 'Cod.', 'Descripcion', 'Cant', 'Und', 'Precio', 'Importe'];
    const columnPositions = [margin, margin + 40, margin + 80, margin + 280, margin + 330, margin + 380, margin + 450];
    ...
    elements.forEach((element, index) => {
        ...
        const description = element.description.length > 30
