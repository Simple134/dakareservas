  ...
  useEffect(() => {
    const fetchEverything = async () => {
      ...
      if (data) {
        ...
        const capitalSoldOut = dakaCapital
          ? dakaCapital.limit - dakaCapital.count <= 0
          : true;
        const plusSoldOut = dakaPlus
        ...
        if (capitalSoldOut && plusSoldOut) {
          ...
          validProducts = data.filter(
            (p) => p.name.toUpperCase() === "RESERVA",
          );
          setShowSoldOutMessage(true);
        } else {
          // At least one available -> Show standard products
          validProducts = data.filter((p) =>
            ["DAKA CAPITAL", "DAKA CAPITAL PLUS"].includes(p.name),
          );
