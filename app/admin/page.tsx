...
type UserViewModel = {
  ...
  raw: Tables<"persona_fisica"> | Tables<"persona_juridica">;
};
...
function AdminPageContent() {
  ...
  const [view, setView] = useState<"reservations" | "locales" | "users">(
    "reservations",
  );
  ...
  const [addUserType, setAddUserType] = useState<"fisica" | "juridica">(
    "fisica",
  );

  // Locale Management State
  const [selectedLocale, setSelectedLocale] =
    useState<Tables<"locales"> | null>(null);
  const [localeOwner, setLocaleOwner] = useState<UserViewModel | null>(null);
  const [assignTab, setAssignTab] = useState<"existing" | "new">("existing");
  const [newUserType, setNewUserType] = useState<"fisica" | "juridica">(
    "fisica",
  );
  ...
  const fetchReservations = async () => {
    try {
      ...
      const transformed: ReservationViewModel[] = (data || [])
        .map((item) => {
          ...
          if (isFisica && item.persona_fisica) {
            ...
            viewModel = {
              ...
              client_name:
                `${pf.first_name || ""} ${pf.last_name || ""}`.trim(),
  ...
  const fetchAllUsers = async () => {
    try {
      ...
      setUsers(
        Array.from(uniqueUsersMap.values()).sort((a, b) =>
      ...
      showAlert(
        "Error",
        ("Error cargando usuarios: " + error) as string,
        "error",
      );
    } finally {
      setLoading(false);
    ...
    if (locale.status !== "DISPONIBLE") {
      ...
      if (allocation) {
        if (allocation.persona_fisica) {
          const pf = allocation.persona_fisica;
          const addressParts = [
            pf.address_street,
            pf.address_house ? `#${pf.address_house}` : "",
            pf.address_sector,
          ...
          const pj = allocation.persona_juridica;
          const addressParts = [
            pj.company_address_street,
            pj.company_address_house ? `#${pj.company_address_house}` : "",
            pj.company_address_sector,
  ...
  const handleUpdateUser = async (
    updatedData: Partial<UserViewModel> &
      Partial<Tables<"persona_fisica">> &
      Partial<Tables<"persona_juridica">>,
  ) => {
    ...
    try {
      const table =
        selectedUser.type === "fisica" ? "persona_fisica" : "persona_juridica";
      ...
      showAlert(
        "Error",
        ("Error actualizando usuario: " + error) as string,
        "error",
      );
    } finally {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showAlert(
          "Error",
          "No se pudo enviar la invitación: " + result.message,
          "error",
        );
      }
    } catch (error: unknown) {
      ...
      showAlert(
        "Error",
        ("Error enviando correo: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
    ...
    try {
      ...
      if (newStatus === "approved" && selectedReservation.locale_id) {
        ...
        if (localeError) {
          ...
          showAlert(
            "Advertencia",
            "Reserva aprobada, pero hubo un error actualizando el estado del local: " +
              localeError.message,
            "warning",
          );
        } else {
          setLocales((prev) =>
            prev.map((l) =>
              l.id === selectedReservation.locale_id
                ? { ...l, status: "VENDIDO" }
                : l,
            ),
          );
      ...
      showAlert("Error", ("Error updating status: " + err) as string, "error");
    } finally {
      setUpdatingStatus(false);
    }
  };
  ...
  const saveLocaleStatus = async (localeId: number, e: React.MouseEvent) => {
    ...
    try {
      ...
      // Update local state
      setLocales((prev) =>
        prev.map((l) => (l.id === localeId ? { ...l, status: newStatus } : l)),
      );
      ...
      showAlert(
        "Error",
        ("Error actualizando estado: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          ...
          showAlert(
            "Error",
            ("Error eliminando reserva: " + err) as string,
            "error",
          );
        } finally {
          setUpdatingStatus(false);
  ...
  const handleUpdatePaymentInfo = async (
    ...
    try {
      ...
      showAlert(
        "Error",
        ("Error registrando pago: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
  ...
  const handleUpdatePaymentStatus = async (
    ...
    try {
      ...
      showAlert(
        "Error",
        ("Error actualizando estado de pago: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
    ...
    try {
      ...
      setSelectedReservation((prev) => {
        ...
        return {
          ...prev,
          payments: prev.payments.map((p) =>
            p.id === paymentId ? { ...p, payment_method: newMethod } : p,
          ),
        };
      });
    } catch (error: unknown) {
      ...
      showAlert(
        "Error",
        ("Error actualizando método de pago: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
  ...
  const handleDeletePayment = async (paymentId: string) => {
    ...
    try {
      ...
      showAlert(
        "Error",
        ("Error eliminando pago: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
  ...
  const handleUploadQuotation = async () => {
    ...
    if (!editQuotationFile) {
      showAlert(
        "Atención",
        "Por favor seleccione un archivo PDF para la cotización.",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      if (selectedReservation.email) {
        ...
        if (emailResult.success) {
          showAlert(
            "Éxito",
            "Cotización subida y enviada al cliente por correo.",
            "success",
          );
        } else {
          ...
          showAlert(
            "Atención",
            "Cotización subida, pero hubo un error enviando el correo: " +
              emailResult.message,
            "warning",
          );
        }
        ...
        showAlert(
          "Éxito",
          "Cotización subida correctamente (Cliente sin correo registrado).",
          "success",
        );
      }
    } catch (error: unknown) {
      ...
      showAlert(
        "Error",
        ("Error subiendo cotización: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
    ...
    showConfirm(
      ...
      async () => {
        ...
        try {
          ...
          showAlert(
            "Error",
            ("Error eliminando cotización: " + error) as string,
            "error",
          );
        } finally {
          setUpdatingStatus(false);
  ...
  const handleUpdateProduct = async (productId: string) => {
    ...
    try {
      ...
      showAlert(
        "Error",
        ("Error actualizando producto: " + error) as string,
        "error",
      );
    } finally {
      setUpdatingStatus(false);
  ...
  const assignLocaleToUser = async (
    userId: string,
    type: "fisica" | "juridica",
  ) => {
    ...
    if (!selectedProductId) {
      showAlert(
        "Atención",
        "Por favor seleccione un producto para la reserva.",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      if (selectedLocale.status === "DISPONIBLE") {
        ...
        setLocales((prev) =>
          prev.map((l) =>
            l.id === selectedLocale.id ? { ...l, status: "BLOQUEADO" } : l,
          ),
        );
      }
      ...
      showAlert(
        "Éxito",
        "Usuario asignado y reserva creada correctamente",
        "success",
      );
    } catch (e: unknown) {
      console.error(e);
      showAlert("Error", ("Error asignando usuario: " + e) as string, "error");
    } finally {
      setUpdatingStatus(false);
    }
  };
  ...
  const createAndAssignUser = async () => {
    ...
    try {
      ...
      showAlert(
        "Éxito",
        "Usuario creado, asignado y reserva generada correctamente",
        "success",
      );
    } catch (e: unknown) {
      console.error(e);
      showAlert("Error", ("Error creando usuario: " + e) as string, "error");
    } finally {
      setUpdatingStatus(false);
    }
  };
    ...
    showConfirm(
      ...
      async () => {
        ...
        if (!localeOwner.allocation_id) {
          showAlert(
            "Error",
            "No se encontró la reserva asociada para desvincular.",
            "error",
          );
          return;
        }
        ...
        if (error) {
          showAlert(
            "Error",
            "Error desvinculando usuario: " + error.message,
            "error",
          );
          return;
        }
        ...
        if (localeError) {
          showAlert(
            "Advertencia",
            "Usuario desvinculado, pero hubo un error actualizando el estado del local: " +
              localeError.message,
            "warning",
          );
        }
        ...
        setLocales((prev) =>
          prev.map((l) =>
            l.id === selectedLocale.id ? { ...l, status: "DISPONIBLE" } : l,
          ),
        );
  ...
  const filteredLocales = locales.filter((locale) => {
    if (levelFilter !== "all" && locale.level.toString() !== levelFilter)
      return false;
    if (
      statusFilter !== "all" &&
      locale.status.toLowerCase() !== statusFilter.toLowerCase()
    )
      return false;
    if (minPriceFilter && locale.total_value < parseFloat(minPriceFilter))
      return false;
    if (maxPriceFilter && locale.total_value > parseFloat(maxPriceFilter))
      return false;
    return true;
  });

  const uniqueLevels = Array.from(new Set(locales.map((l) => l.level))).sort(
    (a, b) => a - b,
  );
  const uniqueStatuses = Array.from(
    new Set(locales.map((l) => l.status)),
  ).sort();
  ...
  const filteredReservations = reservations.filter((res) => {
    const matchesSearch =
      ...
      res.identification_value
        .toLowerCase()
        .includes(searchQuery.toLowerCase());

    const matchesProduct =
      resFilterProduct === "all" || res.product_name === resFilterProduct;
    const matchesCurrency =
      resFilterCurrency === "all" || res.currency === resFilterCurrency;
    const matchesPayment =
      resFilterPaymentMethod === "all" ||
      res.payment_method === resFilterPaymentMethod;
    const matchesStatus =
      resFilterStatus === "all" || res.status === resFilterStatus;
  ...
  const uniqueResProducts = Array.from(
    new Set(reservations.map((r) => r.product_name)),
  )
    .filter(Boolean)
    .sort();
  const uniqueResCurrencies = Array.from(
    new Set(reservations.map((r) => r.currency)),
  )
    .filter(Boolean)
    .sort();
  const uniqueResPaymentMethods = Array.from(
    new Set(reservations.map((r) => r.payment_method)),
  )
    .filter(Boolean)
    .sort();
  const uniqueResStatuses = Array.from(
                  ...
                  <div className="flex bg-gray-100 rounded-lg p-1">
                    <button
                      ...
                      className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${
                        view === "reservations"
                          ? "bg-white text-[#A97 shadow-sm"
                          : "text-gray-600 hovert-gray-900"
                      }`}
                    >
                      Reservas{" "}
                  ...
                  <div className="bg-white p-4 rounded-xl shadow-lg border border-gray-100 flex flex-col md:flex-row gap-4">
                    ...
                    <div className="md:w-40">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setResFilterPaymentMethod(e.target.value)
                        }
                        className="block w-full text-black rounded-md border-gray-300 shadow-sm focus:border-[#A9780F] focus:ring-[#A9780F] sm:text-sm p-2 border"
                      >
                        <option value="all">Todos</option>
                          ...
                          {filteredReservations.map((reservation) => (
                            ...
                            >
                              ...
                              <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">
                                <div className="flex flex-col">
                                  <span>
                                    {reservation.identification_value}
                                  </span>
                                  <span className="text-xs text-gray-400">
                                    {reservation.identification_label}
                                  </span>
                                </div>
                              </td>
                              <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">
                              ...
                              >
                                {reservation.profileId ? (
                                  ...
                                  <div
                                    ...
                                    title="Sin perfil registrado - no puede acceder al dashboard"
                                  >
                ...
                <div key="locales" className="space-y-6">
                  ...
                  <div className="overflow-hidden rounded-xl bg-white shadow-2xl border-2 border-[#A9780F]">
                    <div className="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium text-gray-500">
                        Mostrando: {filteredLocales.length} de {locales.length}
                      </span>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-gray-200">
                            ...
                            >
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  <select
                                    value={
                                      pendingStatusUpdates[locale.id] ||
                                      locale.status
                                    }
                                    onChange={(e) =>
                                      handleStatusChange(
                                    ...
                                    className={`block w-full text-xs font-bold rounded-full px-2 py-1 border-0 ring-1 ring-inset focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6 ${
                                      (
                                        pendingStatusUpdates[locale.id] ||
                                        locale.status
                                      )
                                        ?.toLowerCase()
                                        .includes("disponible")
                                        ? "bg-green-50 text-green-700 ring-g-600/20 focus:ring-green-600"
                                        : (
                                              pendingStatusUpdates[locale.id] ||
                                              locale.status
                                            )
                                              ?.toLowerCase()
                                              .includes("vendido")
                                          ? "bg-red-50 text-red-700 ring-red-600focus:ring-red-600"
                                          : (
                                                ...
                                                ?.toLowerCase()
                                                .includes("reservado")
                                            ? "bg-orange-50 text-orange-700 ring-ora600/20 focus:ring-orange-600"
                                            : "bg-yellow-50 text-yellow-700 ring-yel600/20 focus:ring-yellow-600"
                                    }`}
                                  >
                    ...
                    {filteredLocales.length === 0 && (
                      <div className="p-12 text-center">
                        <p className="text-gray-500 text-lg">
                          No locales found matching filters.
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            <th
                              scope="col"
                              className="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-700 text-center w-10"
                            >
                              Tipo
                            </th>
                            <th
                          ...
                          {users
                            ...
                            .map((user) => (
                              ...
                              >
                                ...
                                <td
                                  className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900 max-w-xs truncate"
                                  title={user.name}
                                >
                                  {user.name}
                                </td>
                      ...
                      users.filter((u) => {
                        const matchesSearch =
                          ...
                          u.email
                            ...
                            .includes(userSearchQuery.toLowerCase());
                        const matchesType =
                          userFilterType === "all" || u.type === userFilterType;
                        const matchesStatus =
                          userFilterStatus === "all" ||
                          u.status === userFilterStatus;
              ...
              {isAddUserModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm overflow-y-auto">
                  <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative">
                    ...
                    <div className="p-8">
                      <h2 className="text-2xl font-bold text-[#131E29] mb-6 border-b pb-4">
                        ...
                        <div className="flex gap-4">
                          <button
                            ...
                            className={`px-4 py-2 rounded-lg font-bold border-2 transition-all ${
                              addUserType === "fisica"
                                ? "border-[#A9780F] bg-[#A97 text-white"
                                : "border-gray-200 text-gray hover:border-[#A9780F]"
                            }`}
                          >
                            Persona Física
                          </button>
