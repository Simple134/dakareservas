...
export async function GET(request: NextRequest) {
    try {
        ...
        if (searchParams.has('divisionId')) params.divisionId = searchParams.get('divisionId');
        ...
        if (searchParams.has('elementsPerPage')) params.elementsPerPage = searchParams.get('elementsPerPage');
        if (searchParams.has('status')) params.status = searchParams.get('status');
        if (searchParams.has('beneficiaryId')) params.beneficiaryId = searchParams.get('beneficiaryId');
        ...
        return NextResponse.json(
            { error: 'Failed to fetch invoices', details: error.message || error.msg },
            { status: 500 }
        );
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log('📦 Payload a enviar:', JSON.stringify(gestionoPayload, null, 2));
