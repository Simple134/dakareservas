...
export default function UserPage() {
  ...
  const [selectedInvestmentForPayment, setSelectedInvestmentForPayment] =
      ...
      try {
        ...
        for (const allocation of allocations) {
          if (allocation.locales_id) {
            ...
            if (localeError) {
              console.error(
                `Error fetching locale ${allocation.locales_id}: `,
                localeError,
              );
              continue;
            }
            ...
            const installments = payments.map((p, index) => ({
              ...
              paymentMethod:
                p.payment_method || allocation.payment_method || "N/A",
  ...
  const handlePaymentSubmit = async (amount: string, file: File) => {
    ...
    try {
      ...
      await sendPaymentNotificationAction(
        userName,
        amount,
          ...
          <SidebarPayment
            ...
            currency={
              dashboardData?.investments.find(
                (inv) => inv.id === selectedInvestmentForPayment,
              )?.paidCurrency || "USD"
            }
          />
        )}
